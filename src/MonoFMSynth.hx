...
class MonoFMSynth
{
    ...
    private var ratio:Float = 12.0;
    private var modulationDepth:Float = 200.0;
    ...
    public function render(output:grig.audio.AudioBuffer):Void
    {
        ...
        var frequency2:Float = frequency * ratio;
        var frequency1:Float = 0.0;
        for (i in 0...output.channels[0].length) {
            frequency1 = frequency + Math.sin(phase2 * 2.0 * Math.PI) * modulationDepth * modEG.level * (currentNote.velocity / 30.0);
    ...
    public function setModulationDepth(modulationDepth:Float):Void
    {
